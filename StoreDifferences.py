...
def store_differences():
	...
	for pair in range(0,36):
		...
		coinbase_ask_price = 0
		coinbase_bid_price = 0
		...
		if len(found_unicoindcx) != 0:
			...
			if len(found_binance) != 0:
				...
				for binance_rec in found_binance:
					...
					binance_time_stamp = binance_rec.get('PriceTimeStamp')
				diff_last_with_binance = abs(binance_last_price - unicoindcx_last_price) 
				actual_diff_last_with_binance = binance_last_price - unicoindcx_last_price
				diff_ask_with_binance = abs(binance_ask_price - unicoindcx_ask_price) 
				actual_diff_ask_with_binance = binance_ask_price - unicoindcx_ask_price
				diff_bid_with_binance = abs(binance_bid_price - unicoindcx_bid_price) 
				actual_diff_bid_with_binance = binance_bid_price - unicoindcx_bid_price
				...
				if max_diff_with_binance > tolerance_for_pair:
					...
					diff_flag ='No'
				newdiffrecord = {
							'DifferenceTimeStamp' : binance_time_stamp,
							'Pair' : thispair,
							'DifferenceWithExchange' : 'Binance',
							'ActualDiffLast' : actual_diff_last_with_binance,
							'ActualDiffAsk' : actual_diff_ask_with_binance,
							'ActualDiffBid' : actual_diff_bid_with_binance,
							'AbsMaxDiff' : max_diff_with_binance,
			...
			if len(found_coinbase) != 0:
				...
				for coinbase_rec in found_coinbase:
					...
					coinbase_time_stamp = coinbase_rec.get('PriceTimeStamp')
				diff_last_with_coinbase = abs(coinbase_last_price - unicoindcx_last_price) 
				actual_diff_last_with_coinbase = coinbase_last_price - unicoindcx_last_price
				diff_ask_with_coinbase = abs(coinbase_ask_price - unicoindcx_ask_price) 
				actual_diff_ask_with_coinbase = coinbase_ask_price - unicoindcx_ask_price
				diff_bid_with_coinbase = abs(coinbase_bid_price - unicoindcx_bid_price) 
				actual_diff_bid_with_coinbase = coinbase_bid_price - unicoindcx_bid_price
				...
				if max_diff_with_coinbase > tolerance_for_pair:
					...
					diff_flag ='No'
				newdiffrecord = {
							'DifferenceTimeStamp' : coinbase_time_stamp,
							'Pair' : thispair,
							'DifferenceWithExchange' :'Coinbase',
							'ActualDiffLast' : actual_diff_last_with_coinbase,
							'ActualDiffAsk' : actual_diff_ask_with_coinbase,
							'ActualDiffBid' : actual_diff_bid_with_coinbase,
							'AbsMaxDiff' : max_diff_with_coinbase,
							'DifferenceIsAboveTolerance' : diff_flag
						}
				newdiffentry = Differences.insert_one(newdiffrecord)
...
if __name__ == '__main__':
	while True:
		store_differences()
		print('Stored differences running:',str(datetime.now()))
